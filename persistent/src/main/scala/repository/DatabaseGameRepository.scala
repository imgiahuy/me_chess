...
/** Database-backed implementation of GameRepository.
  * 
  ...
  */
class DatabaseGameRepository(gameDao: GameDao)(implicit ec: ExecutionContext) extends GameRepository {
    ...
    Await.result(gameDao.findLatest(), timeout) match {
      case Some(state) =>
        state.id match {
          ...
          case None => 
            try {
              Right(createGame(state))
            } catch {
              case e: Exception => Left(s"Failed to load latest game: ${e.getMessage}")
            }
        }
      case None =>

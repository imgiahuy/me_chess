...
class SlickMoveDao(db: Database, tables: Tables)(implicit ec: ExecutionContext) extends MoveDao {
  ...
  private def serializeSpecialMove(sm: scala.Option[SpecialMoveType]): scala.Option[String] = sm match {
    case None                     => None
  ...
  private def deserializeSpecialMove(s: scala.Option[String]): scala.Option[SpecialMoveType] = s match {
    case None | Some("")                    => None
  ...
  override def findById(id: Int): Future[scala.Option[Move]] = {
  ...
  override def findByGameId(gameId: String): Future[scala.List[Move]] = {

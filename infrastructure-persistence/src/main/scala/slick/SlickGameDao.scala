...
class SlickGameDao(db: Database, tables: Tables, playerDao: SlickPlayerDao, moveDao: SlickMoveDao)(implicit ec: ExecutionContext) extends GameDao {
  ...
  override def findById(id: String): Future[scala.Option[PositionState]] = {
  ...
  override def listAll(): Future[scala.List[String]] = {
    val action = tables.games.map(_.id).result
    db.run(action).map(_.toList)
  }

  override def listSummaries(): Future[scala.List[(String, String, Int, Boolean)]] = {
  ...
  override def findAll(): Future[scala.List[PositionState]] = {
  ...
  override def findLatest(): Future[scala.Option[PositionState]] = {
  ...
  private def parseGameResult(resultOpt: scala.Option[String]): MGameResult = {
    resultOpt match {
      case None => Ongoing
  ...
  private def serializeTimeControl(tc: scala.Option[TimeControl]): scala.Option[String] = {
    tc match {
  ...
  private def deserializeTimeControl(strOpt: scala.Option[String]): scala.Option[TimeControl] = {
    strOpt match {
      case None => None
  ...
  private def serializePlayerTime(pt: scala.Option[PlayerTime]): scala.Option[String] = {
    pt match {
  ...
  private def deserializePlayerTime(strOpt: scala.Option[String]): scala.Option[PlayerTime] = {
    strOpt match {
      case None => None
      case Some(str) if str.isEmpty => None
      case Some(str) =>

...
class SlickPlayerDao(db: Database, tables: Tables)(implicit ec: ExecutionContext) extends PlayerDao {
  ...
  override def findById(id: Int): Future[scala.Option[Player]] = {
    val action = tables.players.filter(_.id === id).result.headOption
    db.run(action).map(_.map { case (_, name, _) => Player(name) })
  }

  override def findByName(name: String): Future[scala.Option[Player]] = {
  ...
  override def findAll(): Future[scala.List[Player]] = {
  ...
  override def findLeaderboard(): Future[scala.List[(Player, Int)]] = {

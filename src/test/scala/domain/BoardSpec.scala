...
class BoardSpec extends AnyFunSuite {
  ...
  test("withMove should move a piece to a new position") {
    val board = Board.initial
    val newBoard = board.withMove(Position(0, 1), Position(0, 3)) // pawn move
  ...
  test("withMove should not mutate original board") {
    val board = Board.initial
    val newBoard = board.withMove(Position(0, 1), Position(0, 3))
  ...
  test("withMove should return same board if source is empty") {
    val board = Board.initial
    val newBoard = board.withMove(Position(4, 4), Position(4, 5))
  ...
  test("withMove should capture opponent piece") {
    val board = Board.empty
      .withMove(Position(0, 0), Position(0, 0)) // no-op, just to show chaining
      .copy(
        squares = Map(
    ...
    val newBoard = board.withMove(Position(0, 0), Position(0, 1))
